...
///
/// Both values are constrained to an editable range AND a step grid (block size a
/// multiple of <see cref="BlockSizeStepSamples"/>, buffer a multiple of
/// <see cref="CaptureBufferStepMs"/>); <see cref="Normalize(SamplingSettings)"/> /
/// the per-field normalizers snap-and-clamp any candidate, so an off-step or
/// out-of-range value can never reach the pipeline or the persisted file.
///
    ...
    int CaptureBufferMs)
{
    ...
    /// <summary>
    /// True when both values are within the editable bounds AND on their step grid —
    /// the precondition for a usable, UI-representable block size and capture buffer.
    /// Not persisted.
    /// </summary>
    [JsonIgnore]
    public bool IsValid =>
