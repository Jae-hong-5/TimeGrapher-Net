...
public sealed class BeatSegmentCaptureTests
{
    ...
    [Fact]
    public void Capture_FlagsFullScaleRawPlateauAsClippedSignal()
    {
        var capture = NewCapture();
        const int aSample = 24000;
        ...
        Assert.True((snapshot.Quality & SignalQualityFlags.ClippedSignal) != 0);
        Assert.True((snapshot.Segments[0].Quality & SignalQualityFlags.ClippedSignal) != 0);
    }
