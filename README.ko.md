...
- **테스트·CI 단순** — `Core`가 UI·OS에 무의존이라 단위 테스트가 쉽고(현재 647개), 별도 환경
  없이 기본 러너에서 검증됩니다.
...
UI는 의도적으로 **미니멀하고 기능적**입니다 — 직각의 평면 크롬과, 장식이 아니라 데이터
전달을 위한 차트. 새 UI는 아래 규칙을 따르며, 규칙이 강제되는 토큰·헬퍼를 재사용하세요(새
스타일을 만들지 마세요).

### 원칙

- **직각만 사용.** 모든 도형은 직각 모서리(`border-radius: 0`)이며 둥근 모서리는 쓰지
  않습니다. 기반 `FluentTheme`는 기본적으로 모서리를 둥글게 처리하므로, 이를 `App.axaml`에서
  앱 전역으로 강제합니다: `ControlCornerRadius`/`OverlayCornerRadius` 토큰을 `0`으로 두고,
  `TemplatedControl`·`Border`·`Thumb`·슬라이더 thumb·`CheckBox`에 `CornerRadius="0"`을
  설정합니다. 지역(local) `CornerRadius` 값은 스타일 설정자보다 우선하므로, 마크업이나
  코드에서 0이 아닌 `CornerRadius`를 지정하지 마세요.
- **직선과 단순한 마커.** 플로팅된 데이터 트레이스는 직선의 실선(1–2 px)이며, 요약·강조
  선은 가독성을 위해 더 두껍게 쓸 수 있습니다. 점선·파선은 참조용 가이드(평균선, A/C
  이스케이프먼트 마커, 리뷰 커서)에만 쓰고 주 데이터에는 쓰지 않습니다. 화살촉이나 장식적
  연결선은 없으며, 개별 샘플은 단순한 채워진 원 마커로 표시합니다.
- **차트에 장식 금지.** 차트는 데이터를 전달하므로 시각 효과를 제거합니다: **그림자·3D·
  그라데이션 없음.** 채움은 평면 단색이고(데이터 구간이나 선택 영역을 음영 처리할 때만
  반투명), 그리드는 의도적으로 저대비이며, 앱의 모든 브러시는 단색입니다.
- **기존 토큰 재사용.** 색·브러시·폰트·기본 크기는 한 곳에 정의해 공유합니다 — 새로 만들지
  마세요.
...
| 대상 | 단일 진실 출처 |
|---|---|
| 색·브러시·폰트·크기·직각 모서리 스타일 | `src/TimeGrapher.App/App.axaml` (`ThemeDictionaries` + `Application.Styles`) |
| 그래프 색(하드코딩 금지, `App.axaml`에서 읽음) | `Rendering/PlotThemePalette.cs` (`FromResources`) |
| 그래프 배경·축·그리드 테마 | `Rendering/PlotThemeHelper.cs` (`Apply`) |
| 탭 ID·이름·갱신 주기·그래프 계열 | `Tabs/InfoTabCatalog.cs` |

색은 한 방향으로 흐릅니다 — `App.axaml` → `PlotThemePalette` → 플롯 — 따라서 앱 전체(크롬과
그래프 모두)를 다시 칠하려면 `App.axaml`의 색 블록만 수정하면 됩니다.
...
`dotnet test` 기준 **647개 테스트 전부 통과**(App 343 / Core 280 / WindowsAudio 11 / LinuxAudio 13).
...
| 테스트 | `dotnet test TimeGrapherNet.sln -c Release` (647/647) | ✅ |
