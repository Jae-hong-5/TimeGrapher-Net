...
- **테스트·CI 단순** — `Core`가 UI·OS에 무의존이라 단위 테스트가 쉽고(현재 660개), 별도 환경
  없이 기본 러너에서 검증됩니다.
...
`dotnet test` 기준 **660개 테스트 전부 통과**(App 350 / Core 286 / WindowsAudio 11 / LinuxAudio 13).
...
| 테스트 | `dotnet test TimeGrapherNet.sln -c Release` (660/660) | ✅ |
