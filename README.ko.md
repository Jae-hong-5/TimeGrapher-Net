...
- **테스트·CI 단순** — `Core`가 UI·OS에 무의존이라 단위 테스트가 쉽고(현재 586개), 별도 환경
  없이 기본 러너에서 검증됩니다.
...
- **직선과 단순한 마커.** 데이터 계열은 직선의 실선(1–2 px)입니다. 점선·파선은 참조용
  가이드(평균선, A/C 이스케이프먼트 마커, 리뷰 커서)에만 쓰고 주 데이터에는 쓰지 않습니다.
  화살촉이나 장식적 연결선은 없으며, 개별 샘플은 단순한 채워진 원 마커로 표시합니다.
...
`dotnet test` 기준 **586개 테스트 전부 통과**(App 290 / Core 280 / WindowsAudio 6 / LinuxAudio 10).
...
| 테스트 | `dotnet test TimeGrapherNet.sln -c Release` (586/586) | ✅ |
