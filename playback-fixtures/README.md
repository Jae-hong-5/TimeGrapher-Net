...
|---|---|---|---|---|
| 1 | `1_18000BPH_clean-18000_48000Hz.wav` | Clean synthetic watch at 18,000 BPH, 48 kHz, no added noise, nominal ERROR RATE, nominal beat error. | `detected_bph=18000`, `sync_status=Synced`, ERROR RATE around `+0.0 s/d`, amplitude around `273°`, beat error around `0.0 ms`. | Baseline control case. ERROR RATE/Scope and G06 should show stable, nearly horizontal tic/toc traces with very small separation. G05 should show clear beat-noise waveform and stable A/C markers. |
| 2 | `2_21600BPH_realistic-21600_48000Hz.wav` | Realistic synthetic 21,600 BPH signal at 48 kHz with moderate natural variation/noise model. | `detected_bph=21600`, `sync_status=Synced`, ERROR RATE currently observed around `+2.3 s/d`, amplitude around `275°`, beat error around `0.1 ms`. | Normal non-ideal playback case. GUI should acquire sync and continue updating without warnings for major G06 faults. G05 waveform should look less ideal than fixture 1 but still readable. |
| 3 | `3_28800BPH_noisy-28800_48000Hz.wav` | 28,800 BPH signal at 48 kHz with added broadband noise. | `detected_bph=28800`, `sync_status=Synced`, ERROR RATE around `+0.1 s/d`, amplitude around `273°`, beat error around `0.0 ms`. | Noise-tolerance case. Detection should still sync. G05 is useful here: waveform should show visible noise, while averaged/strip views should remain usable. G06 should not show large beat-error separation. |
| 4 | `4_36000BPH_highrate-36000_96000Hz.wav` | High BPH 36,000 signal at 96 kHz. Shorter beat period and higher sample rate. | `detected_bph=36000`, `sync_status=Synced`, ERROR RATE around `0.0 s/d`, amplitude around `273°`, beat error around `0.0 ms`. | High-BPH and 96 kHz playback case. The app should handle the denser input without deadline degradation. G06 X-axis will fill faster because beats arrive more frequently. |
| 5 | `5_21600BPH_fast-plus30s_48000Hz.wav` | 21,600 BPH signal at 48 kHz with synthetic ERROR RATE of about `+30 s/d`. | `detected_bph=21600`, `sync_status=Synced`, ERROR RATE currently observed around `+29.9 s/d`, amplitude around `273°`, beat error around `0.0 ms`. | G06 slope/fast-watch case. The ERROR RATE trace should trend upward relative to nominal timing. This is useful for checking that positive ERROR RATE readings correspond to positive trace slope. It is not intended to trigger beat-error separation. |
| 6 | `6_21600BPH_beaterror5ms_48000Hz.wav` | 21,600 BPH signal at 48 kHz with large synthetic beat error of `5 ms`. | `detected_bph=21600`, `sync_status=Synced`, ERROR RATE around `+0.0 s/d`, amplitude around `273°`, beat error around `5.0 ms`. | G06 separation-alert case. Tic/toc traces should be visibly separated, and the Beat Error tab should show the separation alert because the current alert threshold is `0.6 ms`. This is the best fixture for testing the larger G06 point markers and adaptive X-axis visibility. |
...
```text
1_18000BPH_clean-18000_48000Hz.wav: detected_bph=18000 sync_status=Synced results=[ERROR RATE   +0.0 s/d | AMPLITUDE 273° | BEAT ERROR  0.0 ms | BPH 18000]
2_21600BPH_realistic-21600_48000Hz.wav: detected_bph=21600 sync_status=Synced results=[ERROR RATE   +2.3 s/d | AMPLITUDE 275° | BEAT ERROR  0.1 ms | BPH 21600]
3_28800BPH_noisy-28800_48000Hz.wav: detected_bph=28800 sync_status=Synced results=[ERROR RATE   +0.1 s/d | AMPLITUDE 273° | BEAT ERROR  0.0 ms | BPH 28800]
4_36000BPH_highrate-36000_96000Hz.wav: detected_bph=36000 sync_status=Synced results=[ERROR RATE   -0.0 s/d | AMPLITUDE 273° | BEAT ERROR  0.0 ms | BPH 36000]
5_21600BPH_fast-plus30s_48000Hz.wav: detected_bph=21600 sync_status=Synced results=[ERROR RATE  +29.9 s/d | AMPLITUDE 273° | BEAT ERROR  0.0 ms | BPH 21600]
6_21600BPH_beaterror5ms_48000Hz.wav: detected_bph=21600 sync_status=Synced results=[ERROR RATE   +0.0 s/d | AMPLITUDE 273° | BEAT ERROR  5.0 ms | BPH 21600]
```
...
|---|---|---|
| `21600BPH_normal_aonly_noise_normal_48k_float.wav` | `detected_bph=21600`, `sync_status=Synced`, ERROR RATE around `+0.6 s/d`, amplitude around `153°`, beat error around `0.0 ms`. | Low-amplitude / A-dominant edge case. Good for checking that sync can still be acquired while amplitude may be outside healthy range. |
| `21600BPH_normal_closecweak_normal_48k_float.wav` | `detected_bph=21600`, `sync_status=Synced`, ERROR RATE around `+0.7 s/d`, amplitude around `319°`, beat error around `0.0 ms`. | Weak/close C-event edge case. Good for amplitude and C-event robustness checks. |
| `21600BPH_normal_noisegap_normal_48k_float.wav` | `detected_bph=21600`, `sync_status=Synced`, ERROR RATE around `+0.7 s/d`, amplitude missing (`---°`), beat error around `0.6 ms`. | Missing/ambiguous amplitude edge case. Good for graceful degradation checks where BPH sync works but amplitude should not be trusted. |
