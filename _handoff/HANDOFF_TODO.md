...
- [x] **C9** test: `dotnet build` + `dotnet test` green; fix fallout; add/adjust tests for changed behavior. Rebased onto latest `origin/main`, preserved the Health beat-error accept-band and verdict-binding fixes, kept Positions tests aligned with the redesign, added Positions hero/table render coverage, and passed the full Release suite (App 689, Core 369, Verify 25, LinuxAudio 22, WindowsAudio 11).
