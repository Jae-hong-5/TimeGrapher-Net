...
| Level 2.4 | 측정 종료 (사용자 종료 / 외부 비정상 종료) |
...
두 트리거가 같은 즉시 정지 시퀀스로 수렴한다: **사용자 종료**(Reset/내부 stop — Live/Playback/Simulation 모두), 또는 **외부 비정상 종료**(Live capture가 device 끊김 등으로 스스로 끝남 — `CaptureEnded` → `StopRunAndRefreshDevices`). 입력·분석 worker를 `TryStop`으로 멈춘 뒤, **녹음 writer가 있으면 `CloseAudio()`(`AudioCloseCheck()`)로 close까지 확인하는 것이 정지 완료 조건이며, close 실패는 `StopFailed`로 이어진다.** 이어서 세션을 무효화하고 Playback/Simulation이면 오디오 상태를 복원한 뒤 `RunState = Stopped`로 전이한다. 외부 비정상 종료였던 경우 정지 성공 후 `RefreshDevices()`로 device 목록을 갱신하고 `LiveAudioStopped` 상태를 보고한다. Simulation은 자연 종료가 없어 사용자 종료 시 `TryStop` interruption으로만 무한 생성 루프가 끝난다.

![Level 2.4 측정 종료 (사용자 종료 / 외부 비정상 종료)](assets/uml25/run-lifecycle-seq-level24.svg)

### Level 2.5 · Playback 자연 종료

Playback은 WAV 파일이 끝나면(`DoneReadingFile`) 현재 세션을 확인·무효화하고 `RunState = Stopping`으로 전이한 뒤 오디오 상태를 복원한다. 이어서 `CompleteInput` → `DrainAndFlushInput`으로 **마지막 frame을 생성해 frame scheduler에 enqueue하며, UI 반영(`HandleAnalysisFrame`)은 GUI가 `Stopped`에 도달한 뒤 도착할 수 있다.** 녹음 writer가 있으면 `AudioCloseCheck()`로 close를 확인한 뒤(close 실패 시 `StopFailed`) `RunState = Stopped`로 전이한다. (Simulation은 무한 생성이라 자연 종료가 없으며, 정지는 Level 2.4를 따른다.)
