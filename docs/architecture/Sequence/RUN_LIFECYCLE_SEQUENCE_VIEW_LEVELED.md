...
Pause 후 Reset 또는 내부 stop 요청으로 입력·분석 worker를 `TryStop`으로 멈추고 세션을 무효화한다. Playback/Simulation이면 오디오 상태를 복원한 뒤 `RunState = Stopped`로 전이한다.

![Level 2.4 사용자 요청 종료](assets/uml25/run-lifecycle-seq-level24.svg)

### Level 2.5 · Playback/Simulation 자연 종료

입력이 끝나면(`DoneReadingFile` / `SimDone`) 현재 세션을 확인·무효화하고, `CompleteInput` → `DrainAndFlushInput`으로 마지막 frame까지 반영한 뒤 정리한다.
