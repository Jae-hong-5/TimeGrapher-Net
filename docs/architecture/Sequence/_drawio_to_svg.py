...
def sesc(s): return s.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")


def drawio_text(s): return re.sub(r"</?span[^>]*>", "", html.unescape(s)).replace("<br>", "\n")
...
def svg_msg(sx, tx, y, label, kind):
    ...
    if kind == "ret":
        ...
        head = arrow(tx, y, dirn, C_CALL, filled)
    mid = (sx + tx) / 2
    w = text_w(label, 11) + 6
...
def svg_self(x0, x1, y1, y2, label):
    label = label.replace("\n", " ")
    ps = f"{x0},{y1} {x1},{y1} {x1},{y2} {x0},{y2}"
    w = text_w(label, 11) + 6
    lx = x1 + 6
    ty = (y1 + y2) / 2
    return (f'<polyline points="{ps}" fill="none" stroke="{C_CALL}" stroke-width="1.4"/>'
            f'{arrow(x0, y2, 1 if x0 >= x1 else -1, C_CALL, True)}'
            f'<rect x="{lx}" y="{ty-7:.1f}" width="{w:.1f}" height="15" fill="#ffffff"/>'
            f'<text x="{lx+3}" y="{ty+4:.1f}" font-size="11" fill="#111827">{sesc(label)}</text>')


def svg_frame(kind, left, top, right, bottom):
    is_ref = kind.split(" ", 1)[0] == "ref"
...
def svg_duration(x, y1, y2, label):
    lines = label.split("\n")
    tick = 18
    ty0 = y1 + 45 - (len(lines) - 1) * 7 + 4
    txt = "".join(f'<text x="{x+10}" y="{ty0+i*14:.0f}" font-size="11" fill="{C_TEXT}">{sesc(t)}</text>'
                  for i, t in enumerate(lines))
    return (f'<path d="M {x-tick} {y1} H {x} V {y2} H {x-tick}" fill="none" '
            f'stroke="{C_FRAME}" stroke-width="1.4"/>{txt}')
...
def _xy(geo, cells, parent):
    x, y = _fnum(geo.get("x")), _fnum(geo.get("y"))
    pgeo = cells.get(parent).find("mxGeometry") if parent in cells else None
    if parent not in ("0", "1", None) and pgeo is not None:
        x += _fnum(pgeo.get("x"))
        y += _fnum(pgeo.get("y"))
    return x, y
...
def render_diagram(diagram):
    ...
    lifelines, frames, dividers, bars, msgs, selfs, texts, durations = [], [], [], [], [], [], [], []
    pid = "page"

    for c in cells:
        style = c.get("style") or ""
        value = drawio_text(c.get("value") or "")
        ...
        if "shape=umlLifeline" in style:
            ...
            x, y = _xy(geo, cell_map, c.get("parent"))
            w, h = _fnum(geo.get("width")), _fnum(geo.get("height"))
            cx = x + w / 2
            ...
            x, y = _xy(geo, cell_map, c.get("parent"))
            w, h = _fnum(geo.get("width")), _fnum(geo.get("height"))
            kind = value if value.split(" ", 1)[0] in FRAME_KINDS else "alt"
            ...
            if sp is None or tp is None:
                continue
            if "duration=1" in style:
                durations.append((sp[0], sp[1], tp[1], value))
            elif "strokeColor=#94a3b8" in style:        # 분기 divider
                dividers.append((sp[0], tp[0], sp[1]))
            elif apoints:                              # self-message (loop)
                selfs.append((sp[0], apoints[0][0], sp[1], apoints[-1][1], value))
            else:                                      # 직선 호출/응답
                kind = "ret" if "endArrow=open" in style and "dashed=1" in style else "async" if "endArrow=open" in style else "call"
    ...
    for x, y1, y2, label in durations:
        max_x = max(max_x, x + max(text_w(t, 11) for t in label.split("\n")) + 12)
    ...
    for x, y1, y2, label in durations:
        out.append(svg_duration(x, y1, y2, label))
...
def main():
    ...
    for diagram in root.findall("diagram"):
        ...
        with open(os.path.join(assets, f"run-lifecycle-seq-{pid}.svg"), "w", encoding="utf-8") as f: f.write(svg)
        n += 1
