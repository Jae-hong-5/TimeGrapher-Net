...
| **sanity checking (캡처 설정)** | Windows Live 입력은 선택된 WinMM 장치 이름을 활성 WASAPI 캡처 엔드포인트와 매칭한 뒤, 공유 표준 sample rate 중 엔드포인트가 지원한다고 보고한 rate만 콤보박스에 노출한다. 동명이 등으로 엔드포인트가 애매하면 임의 선택하지 않아 잘못된 runtime 구성을 시작 전에 차단한다 | `AudioCaptureWorker.cs`, `MainWindow.AudioSetup.cs` | ✓ |
