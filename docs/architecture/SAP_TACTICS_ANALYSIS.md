...
| **sanity checking (캡처 설정)** | Windows Live 입력은 선택된 WinMM 장치 이름을 활성 WASAPI 캡처 엔드포인트와 매칭한 뒤, 공유 표준 sample rate 중 mono shared PCM/float 포맷으로 지원된 rate만 콤보박스에 노출한다. 동명이 등으로 엔드포인트가 애매하거나 rate가 없으면 임의 선택하지 않아 잘못된 runtime 구성을 시작 전에 차단한다 | `AudioCaptureWorker.cs`, `MainWindow.AudioSetup.cs` | ✓ |
...
| **limit structural complexity** | 파서·리듀서·라우터·서비스를 작은 단일책임 단위로 분리, 현재 테스트 소스 87개(앱 45, Core 40, WindowsAudio 1, LinuxAudio 1)가 개별 타깃을 검증 | tests/ | ✓ |
