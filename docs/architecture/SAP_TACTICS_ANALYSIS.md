...
| Bound the stop-teardown budget | Stop/Reset joins the input and analysis worker threads and closes the WAV writer on the UI thread with a short bounded timeout, so a slow-to-terminate capture child (e.g. a USB mic wedged in ALSA/USB I/O) or a slow-draining recording surfaces as StopFailed within ~1 s instead of freezing the window for seconds — the existing StopFailed retry path then recovers. When the WAV writer cannot drain within the budget, its background thread keeps draining and finalizes the WAV header (patches the RIFF/data sizes) when it finishes, so a slow disk yields a retryable StopFailed rather than a corrupted, placeholder-sized recording. Device-enumeration probes (`wpctl`/`arecord`) are time-bounded end-to-end for the same reason. | `RunSessionController.cs`, `QueuedWavStreamWriter.cs`, `LinuxLiveAudioWorker.cs` | ✓ |
