...
같은 깊이에 있는 **Layer 3a(이식 가능 코어)**와 **Layer 3b(외부 기술)**는 서로 사용 관계가 없는 peer다. 둘 다 위 layer(L1·L2)가 사용하지만, `TimeGrapher.Core`는 외부 패키지를 전혀 참조하지 않으므로 L3a → L3b 방향의 사용도 없다. 둘은 상하 관계가 아니라 동급이므로 번호를 3a·3b로 나눈다.
...
flowchart TB
    ...
    subgraph L3["Layer 3 - 이식 가능 코어 · 외부 기술 (서로 사용 관계 없는 peer)"]
        direction LR
        subgraph L3a["Layer 3a - 이식 가능 코어"]
            direction TB
...
| Layer 3a 이식 가능 코어 | `TimeGrapher.Core` 하위 모듈 | UI/OS 비의존적인 시계 음향 분석, WAV I/O, 시뮬레이션, 지표 계산, 공유 프레임 DTO, 오디오 계약 |
| Layer 3b 외부 기술 | Avalonia, ScottPlot, NAudio, PipeWire/ALSA 도구, xUnit | 프로젝트 외부에서 공급되는 프레임워크·OS·플로팅·오디오·테스트 기능 |
...
| L1 → L3a | App·Verify·Core.Tests가 Core 분석/WAV/시뮬레이션/검출 및 `AnalysisFrame`, `BeatMetricsHistorySnapshot` 같은 `Core.Shared` DTO를 사용한다 |
| L1 → L3b | App은 Avalonia·ScottPlot를, 테스트는 xUnit을 사용한다 |
| L2 → L3a | 어댑터가 `Core.Shared`의 라이브 오디오 계약(`ILiveAudioWorker`)을 구현한다 (`AudioCaptureWorker`, `LinuxLiveAudioWorker`) |
| L2 → L3b | WindowsAudio는 NAudio(`NAudio.Wasapi`, `NAudio.WinMM`)를, LinuxAudio는 PipeWire/ALSA 도구(`wpctl`, `pw-record`, `arecord`)를 호출한다 |
...
| L3a ↔ L3b 상호 비의존 | `TimeGrapher.Core`는 외부 패키지를 참조하지 않으므로 L3a → L3b 사용이 없고, 외부 기술도 Core를 모른다. 두 모듈은 상하가 아닌 동급 peer라 번호를 3a·3b로 둔다 |
