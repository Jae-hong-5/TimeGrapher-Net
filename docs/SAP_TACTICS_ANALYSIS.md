...
| **state resynchronization + manage work requests (PLL-guided onset gating)** | BPH/PLL lock 이후에는 sync tracker가 예측한 A phase 안에서만 새 A burst를 열고, 그 guided window에서는 긴 beat period일수록 onset 기준을 더 강하게 적용한다. 이미 재동기화된 시간 기준으로 이른 노이즈 crossing이 한 beat를 선점하지 못하게 막으면서, phase 안의 진짜 약신호는 min-peak 기준 완화로 받아들인다. 측정: weak-2 recall/precision 0.043/0.043 → 0.957/0.957, noisy-1 0.167/0.167 → 0.722/0.722, noisy-2 0.146/0.146 → 1.000/1.000, quiet-step 1.000/1.000 유지 | `TgDetector.cs`, `Detector.cs`, `DetectorStressScenarioTests.cs`, `AdverseScenarios.cs` | ✓ |
