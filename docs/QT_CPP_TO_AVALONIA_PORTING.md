...
이 동작은 Verify의 악조건 행(`--adverse`)으로 측정·게이트된다. 알려진 한계도 같은 하니스에 정직하게 남아 있다: 무음 선행 후 부트스트랩 리셋 폭풍(leadin-quiet 행)은 미해결이고, 극한 잡음에서 PLL 베토는 역효과다(noisy-1 행 — 미래 형태 기반 분류기의 몫).
