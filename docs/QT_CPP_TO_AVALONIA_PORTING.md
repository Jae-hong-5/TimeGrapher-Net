...
원본 Qt/C++ 소스는 임시 제공본으로 권위가 없으며, **"원본 보존"은 더 이상 이 저장소의 원칙이 아니다**(소유자 결정). 검출기는 측정 근거에 따라 자체적으로 진화한다. 약신호/고노이즈 환경 검출력 개선 옵션이 그 첫 사례다: Core 수준에서는 전부 기본 OFF이고, OFF일 때 파이프라인이 옵션 도입 전 기준선과 동일함을 3중 트립와이어가 감시한다(변경 전 기록한 골든마스터 테스트, null vs all-off 옵션 패리티 테스트, Verify `--fidelity-check` CI 게이트 — 목적은 원본 충실도가 아니라 **의도치 않은 동작 드리프트 감지**다). **App 합성 루트는 적응 플로어 + 레짐 가드를 기본 활성**한다(악조건 A/B 측정에서 전 행 무회귀). PLL 이벤트 베토만 GUI 체크박스 뒤 옵트인으로 남는다(극한 지속 잡음에서 recall 회귀가 측정됨). 기준선 경로는 Core 기본값과 Verify baseline 팔이 계속 검증한다.
...
이 옵션들은 기본 검출 설정(`TgConfig`)과 강건성 표면을 분리하기 위해 `TgDetector` 생성자 오버로드(`TgDetectorOptions`)로 들어가며, 조합 효과는 Verify의 악조건 행 A/B(`--ab=baseline,robust`)로 측정·게이트된다. 알려진 한계도 같은 하니스에 정직하게 핀되어 있다: 무음 선행 후 부트스트랩 리셋 폭풍(leadin-quiet 행)은 미해결이고, 극한 잡음에서 PLL 베토는 역효과다(noisy-1 행 — 미래 형태 기반 분류기의 몫).
