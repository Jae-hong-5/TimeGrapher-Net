...
| `manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav` | false-C 전용 테스트 | 앞 1초 warm-up 이후 모든 테스트 beat에서 공통 readout/status, Beat Noise, Waveform Compare, Escapement의 `PossibleFalseC`/`CTimingUnstable` 직접 경고 확인 |
| `manual-fixtures/43200BPH_bad-signal_falseC_weak_192000Hz.wav` | high-rate false-C 보조 테스트 | 43200 BPH에서도 false-C/weak-C warning 경로가 유지되는지 확인 |
...
| `manual-fixtures/28800BPH_transient_falseC_then_clean_192000Hz.wav` | false-C warning recovery | 앞 2초는 false-C 조건을 유도하고 이후 clean 기준으로 돌아와 overlay/status fade-out을 확인 |

Verifier smoke test:

```powershell
dotnet run --project src/TimeGrapher.Verify -c Release -- manual-fixtures/28800BPH_clean_reference_192000Hz.wav manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav manual-fixtures/43200BPH_bad-signal_falseC_weak_192000Hz.wav manual-fixtures/28800BPH_noisy_handling_impulse_192000Hz.wav manual-fixtures/28800BPH_weak_missingC_192000Hz.wav manual-fixtures/28800BPH_clipping_gain_high_192000Hz.wav manual-fixtures/28800BPH_transient_falseC_then_clean_192000Hz.wav
```
...
아래 절차는 그래프 하나를 띄워놓고 `manual-fixtures/*.wav`만 바꿔가며 반복 확인하기 위한 실험 가이드다. 각 테스트는 요청 형식에 맞춰 `1. WAV 실행`, `2. 예상결과`, `3. 실패 기준` 순서로 작성한다. 모든 fixture는 192 kHz Playback 기준이다. `28800BPH_falseC_all_test_beats_192000Hz.wav`는 detector history를 만들기 위한 앞 1초 warm-up 이후 모든 테스트 beat가 false-C 조건이 되도록 재생성했다.
...
1. `manual-fixtures/28800BPH_clean_reference_192000Hz.wav` 실행.
2. 예상결과: 상단 readout에 `Signal ...` suffix가 없고 status bar가 일반 색상으로 정상 측정 상태를 표시한다.
3. 실패 기준: clean 입력인데 warning suffix가 남거나 status bar가 빨간 warning 상태로 남는다.

1. `manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav` 실행.
2. 예상결과: warm-up 이후 상단 readout/status bar가 `Possible false C` 또는 `C timing unstable` 계열 warning을 표시하고, status bar가 빨간색 warning 상태로 Beat Noise 확인 또는 handling noise 감소를 안내한다.
3. 실패 기준: 테스트 구간에서 warning이 없거나 clean 측정처럼 보인다.

1. `manual-fixtures/28800BPH_weak_missingC_192000Hz.wav` 실행.
...
1. `manual-fixtures/28800BPH_clipping_gain_high_192000Hz.wav` 실행.
...
1. `manual-fixtures/28800BPH_noisy_handling_impulse_192000Hz.wav` 실행.
...
1. `manual-fixtures/28800BPH_transient_falseC_then_clean_192000Hz.wav` 실행.
...
1. `manual-fixtures/28800BPH_snr35dB_white_noise_192000Hz.wav` 실행.
2. 예상결과: clean에 가까운 pattern이며 noise floor가 가장 낮다.
3. 실패 기준: clean과 구분되지 않는 수준을 넘어 lock이 깨진 것처럼 보인다.

1. `manual-fixtures/28800BPH_snr25dB_white_noise_192000Hz.wav` 실행.
...
1. `manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav` 실행.
2. 예상결과: warm-up 이후 우측 상단 overlay가 `POSSIBLE FALSE C` 또는 `C TIMING UNSTABLE`을 표시하고 C marker를 정상 repeatability sample처럼 취급하지 않는다.
...
1. `manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav` 실행.
2. 예상결과: warm-up 이후 lane label과 우측 상단 overlay에 false-C/unstable-C warning이 표시되고 `PossibleFalseC` beat는 mean-C guide에서 제외된다.
...
```text
manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav
...
```powershell
dotnet run --project src/TimeGrapher.Verify -c Release -- manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav manual-fixtures/43200BPH_bad-signal_falseC_weak_192000Hz.wav
```
...
Playback mode에서 `manual-fixtures/28800BPH_falseC_all_test_beats_192000Hz.wav`를
