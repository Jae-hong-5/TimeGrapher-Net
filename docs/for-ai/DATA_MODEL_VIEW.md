...
| `AiExplanationRequest`, `MeasurementSummary`, `AiExplanationResult`, `AiExplanationError`, `AiBackendCredentials` | `TimeGrapher.App.Services` | 서버 경유 AI 설명 기능의 App-side 계약. 요청은 `consentGranted=true`, `locale`, `appVersion`, `logText`, 선택적 측정 요약만 직렬화하며 프롬프트·모델·Gemini API 키는 포함하지 않는다. 응답은 백엔드 `requestId`/`explanation`/`model` 또는 안전한 error/message를 담는다. `AiBackendCredentials`는 Basic Auth 입력값을 메모리와 OS credential store 어댑터 사이에서만 전달하며 `AppSettings`/settings.json에는 저장하지 않는다. |
