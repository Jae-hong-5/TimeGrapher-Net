...
| `AiExplanationRequest`, `MeasurementSummary`, `AiExplanationResult`, `AiExplanationError`, `AiBackendCredentials` | `TimeGrapher.App.Services` | 서버 경유 AI 설명 기능의 App-side 계약. 요청은 사용자 동의 후 `consentGranted=true`, `locale`, `appVersion`, 90,000자 이하 `logText`, 선택적 측정 요약만 직렬화하며 프롬프트·모델·Gemini API 키는 포함하지 않는다. 응답은 백엔드 `requestId`/`explanation`/`model` 또는 안전한 error/message를 담고, 앱은 응답 읽기와 Markdown 표시도 bounded 처리한다. `AiBackendCredentials`는 Basic Auth 입력값을 메모리와 OS credential store 어댑터 사이에서만 전달하며 `AppSettings`/settings.json에는 저장하지 않는다. 저장된 credential은 백엔드 요청 성공 뒤에만 갱신해 실패한 로그인 시도가 기존 저장값을 덮어쓰거나 삭제하지 않게 한다. |
