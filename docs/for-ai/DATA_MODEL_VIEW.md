...
class BeatMetricsHistorySnapshot {
    ...
    +MetricsHistorySeries ActivePositionRate
    +MetricsHistorySeries ActivePositionAmplitude
...
BeatMetricsHistorySnapshot "1" *-- "5" MetricsHistorySeries : 전역 rate/amplitude/beat error + 현재 위치 rate/amplitude
...
| `BeatMetricsHistorySnapshot`, `MetricsHistorySeries` | `Core.Shared` (`Core.Metrics.BeatMetricsHistory`가 생성) | 전역 rate/amplitude/beat-error 누적 이력 시리즈(Long-Term·Vario가 소비) **+ 현재 위치 전용 `ActivePositionRate`/`ActivePositionAmplitude`**(Trace 탭 두 그래프). 후자는 위치별 `DecimatingSeries`를 그 위치의 **재기준 경과시간**(다른 위치 체류 시간 제외)으로 누적해 재방문 시 이어붙이고, 전역 시리즈는 그대로 유지된다. 최신 판독값·통계·활성 위치·락 BPH 포함, 프레임 간 공유되며 latest-wins 합병에도 손실 없음 |
| `RateErrorRing` | `Core.Metrics` (`Core.Metrics.BeatMetricsHistory`가 위치별로 소유) | `WatchMetrics`의 250-슬롯 tic/toc rate-error wrap 링 포팅. 위치별로 한 쌍(tic/toc)을 두어 Rate Scope(rate-error 창)·Beat Error 트레이스를 위치별로 그린다. 활성 위치의 두 링은 매 프레임 `RateTic`/`RateToc` `GraphSeriesFrame`으로 발행(스냅샷 스로틀이 아니라 프레임 단위 — 기존 즉시 갱신 유지)되며, 동일 `RateErrorYScale`로 wrap해 전역 트레이스와 점 단위로 일치 |
...
| 집약/합성 | `AnalysisFrame`은 그래프 시리즈·마커·메트릭·선택적 이미지(`PixelBuffer`)로 합성된다. 단, 누적 스냅샷(`BeatMetricsHistorySnapshot`/`BeatSegmentsSnapshot`)은 여러 프레임이 같은 불변 인스턴스를 공유하므로 집약(소유 아님)이다. `BeatMetricsHistorySnapshot`은 `MetricsHistorySeries` 5개(전역 rate/amplitude/beat-error 3개 + 현재 위치 rate/amplitude 2개)와 최대 10개 `PositionSummary`를, `BeatSegmentsSnapshot`은 최대 8개(`SegmentRingCount`)의 `BeatSegment`를 모은다. `BeatNoiseAverageSnapshot`은 현재 평균과 최대 5개의 milestone 평균 스냅샷을 함께 합성해 UI가 Core 내부 누적 배열을 직접 참조하지 않게 한다 |
