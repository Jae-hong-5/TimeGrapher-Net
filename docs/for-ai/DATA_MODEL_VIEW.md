...
| `AnalysisRunSettings` | `TimeGrapher.App` | 사용자가 고른 실행 파라미터(`AnalysisWorker.Config`로 변환). 적응형 floor·regime guard는 기본 동작이다. `AnalysisBlockSize`는 검출기 입력 블록(윈도우) 크기(샘플)로 `Config.AnalysisBlockSize`로 흐른다. `WeakAOnsetRescue`(App "Weak A-onset rescue" 토글, 기본 on)가 꺼지면 `PhaseGuideOnsetRescueScale = 0.0`으로 변환된다. 켜지면 `WeakAOnsetRescueStrengthStep`(0=Safer, 5=Standard, 10=Stronger; 앱 UI/설정 기본 0=Safer, `AnalysisRunSettings` 생성자 생략 기본값은 5=Standard)을 `1.25 - step × 0.05`로 변환해 `PhaseGuideOnsetRescueScale`에 넣고 `DetectorMetricsEngineConfig`→`TgConfig.PhaseGuideOnsetRescueScale`로 흐른다(post-lock 약한 A onset 구제). 같은 방식으로 `SpuriousBeatRejection`(App "Enhanced Auto BPH" 토글, 기본 on)이 켜지면 `AcquisitionPeakGateFraction = 0.35`(꺼지면 0.0)으로 변환되어 `DetectorMetricsEngineConfig`→`TgConfig.AcquisitionPeakGateFraction`로 흐른다(미동기 acquisition 구간에서 하프비트 약한 잡음을 제거해 BPH 2× alias 방지) |
...
| `AiAnalysisRequest`, `MeasurementSummary`, `AiAnalysisResult`, `AiAnalysisError`, `AiAnalysisProgressDisplay`, `AiAnalysisFailureDisplay`, `AiBackendCredentials` | `TimeGrapher.App.Services` | 서버 경유 AI 분석 기능의 App-side 계약. 요청은 사용자 동의 후 `consentGranted=true`, `locale`, `appVersion`, 90,000자 이하 `logText`를 직렬화하며 프롬프트·모델·Gemini API 키는 포함하지 않는다. `MeasurementSummary`/`measurementSummary`는 계약상 예약된 선택 필드지만 현재 앱은 구조화 요약 값을 채우지 않는다. 응답은 백엔드 `requestId`/`explanation`/`model` 또는 안전한 error/message를 담고, 앱은 응답 읽기와 Markdown 표시도 bounded 처리한다. `AiAnalysisProgressDisplay`/`AiAnalysisFailureDisplay`는 요청 직후 먼저 열린 AI Analysis 창의 pending/error 상태를 같은 창 안에서 갱신하기 위한 UI-side 상태 계약이다. `AiBackendCredentials`는 Basic Auth 입력값을 메모리와 OS credential store 어댑터 사이에서만 전달하며 `AppSettings`/settings.json에는 저장하지 않는다. 저장된 credential은 백엔드 요청 성공 뒤에만 갱신해 실패한 로그인 시도가 기존 저장값을 덮어쓰거나 삭제하지 않게 한다. |
