...
두 신호가 같은 비트에서 일치하면 B->A로 본다. 측정은 verifier의 `--diagnose` 모드로 수행한다
(아래 "재현").
...
- mine_adapter는 늦은 비트가 너무 많아 격자 fit 자체가 오염된다(잔차 ±양방향). 그 경우
  **A->C 짧음(42%)** 을 주신호로 본다.
- 진단 도구는 verifier `--diagnose`로 정식 편입됐다(`01517b4`). 학습된 모델이 생기면
  `--landmark=onnx`와 함께 돌려 late-A outlier 감소를 같은 측정으로 비교한다.

## 재현

```powershell
# off (현재 detector)
dotnet run --project src/TimeGrapher.Verify -c Release -- sample/21600BPH_NH35.wav sample/mine.wav sample/mine_adapter.wav --diagnose
# 학습된 모델 확보 후 off vs refiner 비교
dotnet run --project src/TimeGrapher.Verify -c Release -- sample/mine_adapter.wav --diagnose --landmark=onnx:model.onnx
```
