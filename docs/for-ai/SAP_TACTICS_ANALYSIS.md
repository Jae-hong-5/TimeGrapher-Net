...
| Support user initiative | Knobs that do not change a run's structure stay adjustable mid-run instead of forcing a stop/restart: live-capture gain is forwarded to the worker during capture, and the simulation's Error Rate / Amplitude / Beat Error are re-read by the already-running synth stream (a pending value applied on the sim thread between fill blocks). BPH, sample rate, and the Realistic preset stay start-only because they fix the run's structure. | `RunSessionController.cs`, `SimWorker.cs`, `WatchSynthStream.cs`, `MainWindowSelectionCoordinator.cs` | ✓ |
