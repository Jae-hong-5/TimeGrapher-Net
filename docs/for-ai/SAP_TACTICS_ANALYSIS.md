...
| Bound the stop-teardown budget | Stop/Reset joins the input and analysis worker threads and closes the WAV writer synchronously on the UI thread; each wait uses a short bounded timeout so a slow-to-terminate capture child (e.g. a USB mic wedged in ALSA/USB I/O) surfaces as StopFailed within ~1 s instead of freezing the window for seconds — the existing StopFailed retry path then recovers. Device-enumeration probes (`wpctl`/`arecord`) are time-bounded for the same reason. | `RunSessionController.cs`, `QueuedWavStreamWriter.cs`, `LinuxLiveAudioWorker.cs` | ✓ |
