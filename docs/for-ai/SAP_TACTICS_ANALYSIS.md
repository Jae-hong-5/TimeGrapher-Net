...
| Sanity checking | Live device/sample-rate choices are probed before use. On Linux/Raspberry Pi, PipeWire capture sources are matched back to their ALSA hardware when possible, first through `wpctl inspect` ALSA card/device metadata and then by device-name fallback, so the sample-rate menu is narrowed by the hardware `arecord` probe and rejects ALSA's "requested high rate, got lower rate" negotiation instead of accepting every rate that PipeWire can resample. If a PipeWire source cannot be mapped to ALSA, the UI exposes only the conservative 48 kHz default after a successful PipeWire 48 kHz probe instead of either showing nothing or treating every resampled rate as hardware-supported. Live-device cache misses keep the rate list empty until the probe returns, rather than briefly showing unverified rates. Settings loaded from JSON are validated before replacing current defaults. AI backend base URLs are constrained to the approved HTTPS allowlist, logs are checked against the 90,000-character client limit before upload, and credential persistence is enabled only after an OS credential-store store-and-read round-trip probe succeeds (the probe also issues a delete/clear, but that cleanup is best-effort and its result is not asserted). | `AudioDeviceController.cs`, `AudioCaptureWorker.cs`, `LinuxLiveAudioWorker.cs`, `AppSettingsStore.cs`, `AiAnalysisController.cs`, `AiAnalysisService.cs`, `AiCredentialStore.cs` | ✓ |
