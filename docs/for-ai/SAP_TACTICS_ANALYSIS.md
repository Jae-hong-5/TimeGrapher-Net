...
| **startup transient 억제 (완료 구간 판독)** | 상단바 Error Rate/Amplitude/BEAT ERROR는 Settings의 Avg. Period 구간이 완료될 때까지 무효로 보류한다. 완료 시 Error Rate는 tic-to-tic/toc-to-toc 같은 위상 period 차이 평균을 s/24h로 변환하고, Amplitude는 tic/toc 쌍평균 진폭의 구간 평균, BEAT ERROR는 절대 beat error의 구간 평균을 표시한다. 다음 구간이 완료될 때까지 마지막 완료 구간 값을 유지한다. 그래프/히스토리 이벤트 시리즈는 기존 이벤트 샘플(RLS rate, 부호 beat error, tic/toc 쌍평균 진폭)을 계속 사용해 상단바 smoothing과 분리하되, 완료된 Error Rate 구간은 `AveragePeriodRateInterval`로 누적해 Rate/Scope와 Beat Error 그래프에 구간 overlay와 s/d 값으로 노출한다. 새 싱크 락과 검출 갭 복구가 같은 구간 완료 조건을 공유하며, 측정 중간의 미완료 구간은 표시값을 흔들지 않는다. 측정: 깨끗한 nominal 비트에서 기본 2초 Avg. Period가 끝나는 시점에 첫 유효 판독이 발생 | `WatchMetrics.cs`, `BeatMetricsHistory.cs`, `AveragePeriodRateOverlay.cs`, `WatchMetricsDerivedMeasuresTests.cs` | ✓ |
...
| **on-device ML classification (TinyML 신호품질 게이트, 옵트인)** | `OnnxBeatEventGate`(leaf `TimeGrapher.Inference`, ONNX Runtime): 검출된 각 후보의 128점 `BeatWindowFeatures` envelope 윈도우를 온디바이스 ONNX 분류기로 *good escapement vs bad data*로 분류해 나쁜 후보를 메트릭 도달 전에 veto — 프로젝트 명세 AI Feature의 *Bad Data Rejection / Signal Quality Classification*을 온디바이스로 실현. `PllMatchGate`와 같은 `IBeatEventGate` 소켓·drop-only·검출 다운스트림이라 BPH/PLL을 못 흔든다(구조적 보장 — 오분류는 후보 1개 통과/드롭에 갇힘). 모델은 dev 전용 `tools/TimeGrapher.GateTrainer`가 real 녹음으로 학습(clean watch=good, mine_false의 톡뒤 침입자=bad, cadence pseudo-label; SDCA 로지스틱 회귀→ONNX export, held-out AUC 0.994). 측정(real `--ab`): mine_false veto 36.5% vs clean mine_usb/mine 1.9~2.3% — bad 녹음과 좋은 시계 사이 ~18배. ⚠ real로 학습해 *합성* adverse는 분포 밖이라(weak/noisy 과veto, postc-noise 무veto) `--adverse --gate=onnx`는 INFO 측정 — sim↔real envelope 격차는 문서화된 제약이고 real 라벨 확대가 일반화의 관건 | `OnnxBeatEventGate.cs`, `BeatWindowFeatures.cs`, `tools/TimeGrapher.GateTrainer/`, `GateAbDemo.cs`, `AdverseScenarios.cs` | ✓ |
