...
| Limit event response | The render scheduler keeps the latest frame and coalesces intermediate frames. Active live graph tabs drain at a display-rate 16 ms cadence, while image-heavy Sound Print/Spectrogram stay on their 100 ms publish cadence. One-shot signals such as input overrun are merged so dropping intermediate UI frames does not drop important state. | `AnalysisFrameRenderScheduler.cs`, `InfoTabCatalog.cs` | ✓ |
