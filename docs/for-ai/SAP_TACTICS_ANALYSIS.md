...
| **fault detection via sustained inconsistency (false-lock 거부)** | sync는 주기성(Rayleigh phase score)만으로 락하므로 불규칙한 비-시계 임펄스 열(예: 잡음)도 락될 수 있고, 락 후 연속-miss 카운터(`SyncLossMisses`=12)는 간헐 in-phase 이벤트가 매번 리셋해 가짜 락을 무한정 유지한다(연속-miss는 *지속* 산포를 못 본다). `TgSync`가 락 이후 A-A 간격 64개 창을 유지해, 창 중앙값 대비 ±25% 밴드 밖 간격이 >20%면(=in-band<80%) 보유한 락을 demote한다(시간 watchdog과 동일하게 `Synced=0` → 기존 transition 핸들러가 정리). 창은 매 `Lock`/`Reset`에서 비워져 64개 락-후 A 이벤트 전에는 발화하지 않아 획득을 굶기지 않는다(연속-miss 카운터와 독립인 *윈도우* 신호). 측정(캡처 샘플 windowed): 진짜 시계는 흔들리는 어댑터 픽업까지 in-band 100%(CV ≤2.5%)로 유지, 불규칙 false 열은 in-band ~65%(CV ~45%)로 demote. 회귀: `Verify --adverse`의 genuine 행 전부 sync_losses=0, noise-only는 NotSynced 유지(`--generated`/`--byte-fixtures` 전부 Synced) | `TgDetector.cs`, `Bph.cs`, `TgSyncConsistencyTests.cs`, `AdverseScenarios.cs` | ✓ |
