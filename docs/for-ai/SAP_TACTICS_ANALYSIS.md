...
| Sanity checking | Live device/sample-rate choices are probed before use. Settings loaded from JSON are validated before replacing current defaults. AI backend base URLs are constrained to the approved HTTPS allowlist, and credential persistence is enabled only after an OS credential-store store/read/delete probe succeeds. | `AudioCaptureWorker.cs`, `LinuxLiveAudioWorker.cs`, `AppSettingsStore.cs`, `AiExplanationService.cs`, `AiCredentialStore.cs` | ✓ |
